...
const OrderService = {
  placeOrder: async (userId, cartItemsFromClient, shippingInfoFromController, clientTotalAmount) => {
    ...
    try {
      ...
      const productsDetailsWithStore = await Promise.all(
        cartItemsFromClient.map(async (cartItem) => {
          const product = await ProductModel.findById(cartItem.productId)
            ...
            .session(session);
          if (!product || !product.isActive) throw new Error(`Sản phẩm ID ${cartItem.productId} không hợp lệ hoặc đã ngừng kinh doanh.`);
          if (!product.storeId || !product.storeId._id) throw new Error(`Sản phẩm ${product.name} không có thông tin cửa hàng hợp lệ.`);
          if (product.inventory < cartItem.quantity) throw new Error(`Sản phẩm ${product.name} không đủ tồn kho (còn ${product.inventory}, cần ${cartItem.quantity}).`);
      ...
      for (const storeId in itemsByStore) {
        ...
        for (const cartItem of storeCartItems) {
          ...
          if (cartItem.appliedVoucherId && cartItem.voucherDetailsAtOrder) {
            const voucher = await Voucher.findById(cartItem.appliedVoucherId).session(session);
            // Validate voucher với item và store hiện tại
            if (voucher && voucher.isActive && new Date(voucher.startDate) <= new Date() && new Date(voucher.endDate) >= new Date() && (voucher.usedQuantity || 0) < voucher.totalQuantity) {
              ...
              if (voucher.storeId && voucher.storeId.toString() !== storeId) {
                  ...
                  console.warn(`Voucher ${voucher.code} (store: ${voucher.storeId}) không áp dụng cho đơn hàng của store ${storeId}.`);
              }
              ...
              if (isVoucherValidForThisItem && voucher.minPurchaseAmount && itemOriginalSubTotal < voucher.minPurchaseAmount) {
                  isVoucherValidForThisItem = false;
                  console.warn(`Item "${product.name}" chưa đủ điều kiện cho voucher ${voucher.code} (minPurchase).`);
              }
              // 3. Kiểm tra applicableCategories
              if (isVoucherValidForThisItem && voucher.applicableCategories && voucher.applicableCategories.length > 0) {
                const productCategoryStrings = product.categories.map(c => c.toString());
                const voucherCategoryStrings = voucher.applicableCategories.map(c => c.toString());
                if (!productCategoryStrings.some(pc => voucherCategoryStrings.includes(pc))) {
                    isVoucherValidForThisItem = false;
                }
              }
              // 4. Kiểm tra applicableProducts
              if (isVoucherValidForThisItem && voucher.applicableProducts && voucher.applicableProducts.length > 0) {
                const voucherApplicableProductStrings = voucher.applicableProducts.map(p => p.toString());
                if (!voucherApplicableProductStrings.includes(product._id.toString())) {
                    isVoucherValidForThisItem = false;
                }
              }
              ...
              if (isVoucherValidForThisItem) {
                ...
                if (voucher.discountType === 'fixed') {
                  ...
                  discountCalculatedAtBackend = (itemOriginalSubTotal * voucher.discountValue) / 100;
                  if (voucher.maxDiscountAmount && discountCalculatedAtBackend > voucher.maxDiscountAmount) {
                    discountCalculatedAtBackend = voucher.maxDiscountAmount;
                  }
                }
                itemDiscountAmount = Math.min(discountCalculatedAtBackend, itemOriginalSubTotal);
                ...
                overallAppliedVoucherIds.add(voucher._id.toString()); // Thêm vào set để cập nhật sau
                  ...
                  console.warn(`Voucher ${voucher.code} không được áp dụng cho sản phẩm ${product.name} trong đơn hàng của store ${storeId} do không thỏa mãn điều kiện.`);
              }
            } else if (voucher) {
              console.warn(`Voucher ${voucher.code} không hợp lệ hoặc đã hết lượt/hết hạn (cho item ${product.name}).`);
            } else {
              console.warn(`Voucher ID ${cartItem.appliedVoucherId} không tìm thấy (cho item ${product.name}).`);
            }
          }
        ...
        const currentOrderTotalAmount = currentOrderSubTotal - currentOrderTotalDiscount;
      ...
      for (const voucherIdStr of overallAppliedVoucherIds) {
        const voucherToUpdate = await Voucher.findById(voucherIdStr).session(session);
        if (voucherToUpdate) {
          if ((voucherToUpdate.usedQuantity || 0) < voucherToUpdate.totalQuantity) {
          ...
          const userVoucherInstance = userDocument.vouchers.find(
            (v) => v.voucherId && v.voucherId.equals(voucherToUpdate._id) && !v.isUsed
          );
          if (userVoucherInstance) {
            ...
            console.warn(`Không tìm thấy bản ghi user_voucher chưa sử dụng cho voucherId ${voucherIdStr} của user ${userId}`);
          }
        }
      }
      ...
      session.endSession();
      console.error("Error in OrderService.placeOrder:", error.message, error.stack);
  ...
  getOrdersByUserId: async (userId) => {
    try {
      const orders = await Order.find({ userId: userId })
        .populate({ path: "items.productId", model: "Product", select: "name price images" }) // Bỏ storeId ở đây vì đã có ở cấp Order
        .populate({ path: "storeId", model: "Store", select: "name logo" })
        .populate({ path: "items.appliedVoucherInfo.voucherId", model: "Voucher", select: "code description" })
        .sort({ createdAt: -1 });
      return orders;
    } catch (error) {
  ...
  getOrderById: async (orderId, userId) => {
    try {
      const order = await Order.findOne({ _id: orderId, userId: userId })
        .populate({ path: "items.productId", model: "Product", select: "name price images" })
        .populate({ path: "storeId", model: "Store", select: "name logo" })
        .populate({ path: "items.appliedVoucherInfo.voucherId", model: "Voucher", select: "code description discountType discountValue" });
      return order;
    } catch (error) {
